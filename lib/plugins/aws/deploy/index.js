...
class AwsDeploy {
  constructor(serverless, options) {
    ...
    this.hooks = {
      ...
      'aws:deploy:deploy:createStack': () => BbPromise.bind(this).then(this.createStack).catch(err => {console.log('create stack err', err); throw err}),

      'aws:deploy:deploy:checkForChanges': () =>
        BbPromise.bind(this).then(this.setBucketName).then(this.checkForChanges).catch(err => {console.log('checkForChanges err', err); throw err}),
      ...
      'aws:deploy:deploy:uploadArtifacts': () =>
        ...
        }).catch(err => {console.log('uploadArtifacts err', err); throw err}),
      ...
      'aws:deploy:deploy:validateTemplate': () =>
        ...
        }).catch(err => {console.log('validateTemplate err', err); throw err}),
      ...
      'aws:deploy:deploy:updateStack': () =>
        ...
        }).catch(err => {console.log('updateStack err', err); throw err}),
